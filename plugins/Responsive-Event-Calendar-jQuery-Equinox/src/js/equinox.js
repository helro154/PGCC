...
(function($) {
	...
	var getMonthHtml = function(month, events) {
		...
		$('.calendar-actions', header).append('<button type="button" class="prev"><svg fill="#002c77" height="30" viewBox="0 0 24 24" width="30" xmlns="http://www.w3.org/2000/svg"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></button> ');
		$('.calendar-actions', header).append('<div class="month-label">' + month.format('MMMM YYYY') + '</div> ');
		$('.calendar-actions', header).append('<button type="button" class="next"><svg fill="#002c77" height="30" viewBox="0 0 24 24" width="30" xmlns="http://www.w3.org/2000/svg"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></button> ');
	...
	var getDateHtml = function(date, events, reservedSlots, activeMonth, currentDate) {
		...
		dateHtml.append('<div class="date-label">'+ (date.date() === 1 ? date.format('MMM ') : '')+'<span>' + (date.date() === 1 ? date.format('MMM ') : '') + date.date() + '</span></div>');
