<?php
//use Calendar\Core\Container as Container;

/* Version 0.1.126 */
$t4_config['phar'] 		= '/media/sample-site---php-events-calendar/events-calendar-source.phar';
$t4_config['sources']	= 'TerminalFourEvents';
$t4_config['maxevents']	= '10';
$t4_config['allevents']	= '/alumni/events/calendar/';
$t4_config['jsonxml']	= '/alumni/events/calendar/event-calendar.json';
$t4_config['gapi']		= '';
$t4_config['gids']		= '';
$t4_config['icals']		= '';
$t4_config['language'] 	= '';
$t4_config['timezone'] 	= 'Europe/Dublin';
$t4_config['options']	= 'cache_directory = /tmp/events_calendar_php/
google_cache_refresh_time = 60 minutes';


if (!class_exists('T4EventsCalendar')) {
	class T4EventsCalendar
	{
		static public $runConfig = false;

		static protected $variables_to_skip = array("all_event_url","calendar_language","timezone","enabled_sources","events_data_source","google_client_email","google_api_p12_token_file","google_calendar_id","ical_source_location","max_pagination");

		static protected $configSettings = array();
		...
		public static function config($t4_config) {
			...
			if(!is_dir(self::$configSettings["view_directory"])  && empty($isInOptions)) {
				throw new Exception("Add views directory in the same directory of the PHAR file");
			}
			...
			if (!empty($options)) {
			    \Calendar\Core\Config::setFromString($options,self::$variables_to_skip,true);
			}
		}
		...
		public static function module($t4_config, $t4_module, $force_view = null) {
		    ...
		    if (!empty($module_options)) {
		        if(isset(self::$variables_to_skip)) {
		            \Calendar\Core\Config::setFromString($module_options,self::$variables_to_skip,false);
		        }
		    }
		    ...
		    $class = 'Calendar\Modules\\'.str_replace('extended.','',$class);
				if ($class != 'Calendar\Modules\Download') {
